...
export class SignupService {
  async signup(): Promise<string> {
    const browser = await puppeteer.launch({ headless: false, defaultViewport: null, args: ['--no-sandbox', '--disable-setuid-sandbox', '--incognito'] });
    ...
    try {
      ...
      const detailsSelector = 'details.dropdown[data-category="cta"][data-action="create an account dropdown"]';
